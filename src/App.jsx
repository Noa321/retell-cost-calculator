...
// Retell.ai pricing data
const VOICE_ENGINES = {
  'eleven_labs': { name: 'ElevenLabs', rate: 0.070 },
  'eleven_flash_v2': { name: 'ElevenLabs Flash v2', rate: 0.060 },
  'eleven_turbo_v2_5': { name: 'ElevenLabs Turbo v2.5', rate: 0.055 },
  'play_ht': { name: 'Play.ht 3.0 Mini', rate: 0.060 },
  'play_dialog': { name: 'PlayDialog', rate: 0.065 },
  'deepgram': { name: 'Deepgram', rate: 0.050 },
};

const LLM_MODELS = {
  'gpt_4o': { name: 'GPT-4o', rate: 0.045 },
  'gpt_4o_mini': { name: 'GPT-4o Mini', rate: 0.035 },
  'gpt_4_1': { name: 'GPT 4.1', rate: 0.045 },
  'gpt_4_1_mini': { name: 'GPT 4.1 Mini', rate: 0.030 },
  'gpt_5_2': { name: 'GPT 5.2', rate: 0.056 },
  'claude_3_5_sonnet': { name: 'Claude 3.5 Sonnet', rate: 0.050 },
  'claude_3_7_sonnet': { name: 'Claude 3.7 Sonnet', rate: 0.055 },
  'gemini_2_flash': { name: 'Gemini 2.0 Flash', rate: 0.027 },
};

const TELEPHONY = {
  'twilio_us': { name: 'Twilio US', rate: 0.015 },
  'twilio_toll_free': { name: 'Twilio Toll-Free', rate: 0.022 },
  'twilio_intl': { name: 'Twilio International', rate: 0.045 },
  'bandwidth': { name: 'Bandwidth', rate: 0.012 },
  'vonage': { name: 'Vonage', rate: 0.014 },
};
...
export default function App() {
  const [voiceEngine, setVoiceEngine] = useState('eleven_labs');
  const [llmModel, setLlmModel] = useState('gpt_4_1');
  const [telephony, setTelephony] = useState('twilio_us');
  ...
  const calculations = useMemo(() => {
    ...
    const nonLlmPerMin = voicePerMin + phonePerMin + kbPerMin + vcPerMin + piiPerMin;
    
    const calculateForDuration = (duration) => {
      const totalTokens = dashboardTokens + (growthRate * duration);
    ...
    const minCalc = calculateForDuration(minCallDuration);
    const avgCalc = calculateForDuration(avgCallDuration);
    const maxCalc = calculateForDuration(maxCallDuration);
    
    const rangeTable = [];
    for (let d = minCallDuration; d <= maxCallDuration; d += 1) {
  ...
  }, [voiceEngine, llmModel, telephony, useKnowledgeBase, useVoiceCancellation, usePII, dashboardTokens, transcriptGrowth, minCallDuration, maxCallDuration, avgCallDuration, callsPerDay, daysPerMonth]);
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-r from-orange-900/50 to-yellow-900/50 border border-orange-600 rounded-lg p-4 md:p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setDashboardTokens(Number(e.target.value))}
                    min="0"
              ...
              <div className="mt-4 flex items-center gap-3">
                <div className="flex-1 bg-gray-800 rounded-full h-3 overflow-hidden">
                  <div 
                    className={`h-full transition-all ${dashboardTokens > TOKEN_THRESHOLD ? 'bg-red-500' : dashboardTokens > 2500 ? 'bg-yellow-500' : 'bg-green-500'}`}
                    style={{ width: `${Math.min(100, (dashboardTokens / TOKEN_THRESHOLD) * 100)}%` }}
                  />
                </div>
                <span className="text-sm whitespace-nowrap">
                  {dashboardTokens > TOKEN_THRESHOLD ? (
                    <span className="text-red-400">⚠️ Over 3,500!</span>
                  ) : (
                    <span className="text-gray-400">{TOKEN_THRESHOLD - dashboardTokens} until surcharge</span>
                  )}
                </span>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-4 mt-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    className="w-4 h-4 rounded accent-blue-500"
                  />
                  <span className="text-sm">PII Redaction ($0.01/min)</span>
                </label>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {calculations.rangeTable.map((row) => (
                      <tr 
                        key={row.duration} 
                        className={`border-b border-gray-700 ${row.duration === avgCallDuration ? 'bg-green-900/30' : ''}`}
                      >
                        <td className="py-2 px-2">
                          {row.duration} min
                          {row.duration === avgCallDuration && <span className="text-green-400 text-xs ml-1">(avg)</span>}
                        </td>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="font-semibold text-gray-300 mb-3">Per Call Estimates</h3>
              
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-gray-700">
                  <span className="text-gray-400">Min ({minCallDuration} min)</span>
                  <span className="text-white font-semibold">{formatCurrency2(calculations.minCalc.totalCost)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-700 bg-green-900/20 -mx-4 px-4">
                  <span className="text-green-400">Avg ({avgCallDuration} min)</span>
                  <span className="text-green-400 font-bold text-lg">{formatCurrency2(calculations.avgCalc.totalCost)}</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-gray-400">Max ({maxCallDuration} min)</span>
                  <span className="text-white font-semibold">{formatCurrency2(calculations.maxCalc.totalCost)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-orange-900/30 border border-orange-600 rounded-lg p-4">
              ...
              <div className="text-sm space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{dashboardTokens.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">+ Transcript ({avgCallDuration} min):</span>
                  <span className="text-white">+{(calculations.growthRate * avgCallDuration).toLocaleString()}</span>
                </div>
            ...
            <div className="bg-gray-800 rounded-lg p-4 text-xs">
              <h3 className="font-semibold mb-2 text-gray-300">💡 Cost Reduction Tips</h3>
              <ul className="space-y-1 text-gray-400">
                ...
                <li>• Gemini Flash is 40% cheaper than GPT</li>
        ...
        <div className="mt-6 text-center text-xs text-gray-500">
          <p>Built for Retell.ai voice agent cost estimation • Pricing data as of January 2026</p>
        </div>
      </div>
    </div>
  );
